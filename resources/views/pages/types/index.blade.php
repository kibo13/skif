...
<!-- type-index -->
@section('content')
<section id="type-index" class="section">
        ...
        <td>
          <div class="bk-types">
            <img 
              ...
              alt="{{ $product->name }}" >
            <div class="bk-types__info">
              <h6 class="bk-types__title mb-0">
                {{ $type->product->name }}
              </h6>
              <p class="bk-types__item text-muted mb-1">
                ({{ $type->product->L . 'x' . $type->product->B . 'x' . $type->product->H}})
              </p>
              <p class="bk-types__item">
                <span class="bk-types__subtitle">Артикул:</span> 
                {{ $type->product->code }} 
              ...
              <p class="bk-types__item">
                <span class="bk-types__subtitle">Материал:</span> 
                @if($type->product->category->slug == 'soft')
                Экокожа
                @else 
                ЛДСП
              ...
              <p class="bk-types__item">
                <span class="bk-types__subtitle">Цвет:</span>
                @if($type->product->category->slug == 'soft')
                {{ $type->fabric->name }}
                @else
                {{ $type->plate->name }}
